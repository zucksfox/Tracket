...
<div class="max-w-2xl mx-auto space-y-5">
    <div>
        <a href="{{ route('technicians.index') }}" class="text-[12px] hover:underline" style="color: var(--act);">Kembali ke Data Teknisi</a>
        <h1 class="text-[20px] font-bold t-ink tracking-tight mt-1">Registrasi Akun Teknisi / Petugas</h1>
        <p class="text-[12.5px] t-muted mt-0.5">Akun baru dapat langsung dipakai login di halaman masuk setelah disimpan.</p>
    </div>
        ...
        <form action="{{ route('technicians.store') }}" method="POST" class="space-y-4">
            ...
            <div>
                ...
                <input type="email" id="email" name="email" value="{{ old('email') }}" required
                    placeholder="contoh: budi@tracket.test" class="field mono">
                <span class="text-[11px] t-muted mt-1 block">Email ini dipakai bersama kata sandi untuk masuk ke sistem.</span>
            </div>

            <div>
                <label for="password" class="block mb-1">Kata Sandi Awal <span style="color: var(--rose);">*</span></label>
                <input type="password" id="password" name="password" required minlength="8"
                    placeholder="minimal 8 karakter" class="field mono">
                <span class="text-[11px] t-muted mt-1 block">Tersimpan sebagai hash Bcrypt. Beritahu pengguna agar menggantinya setelah login pertama.</span>
            </div>
